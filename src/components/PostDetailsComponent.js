			...
			{postDetails &&
				<div className="card ml-4 mt-4 mb-4" key={postDetails.id}>
					<div className="card-header">
						<h5 className="card-title text-capitalize text-left">{postDetails.title}</h5>
					</div>
					<div className="card-body">
						<p className="card-text text-capitalize text-left">{postDetails.body}</p>
					</div>
				</div>
			}
			...
			{postComments && postComments.length !== 0 &&
				postComments.map(function(element, index) {
					return  <Card key={index}>
								<Accordion.Toggle className="text-capitalize text-left" as={Card.Header} eventKey={index + 1}>
								  {element.name}
								</Accordion.Toggle>
								<Accordion.Collapse eventKey={index + 1}>
								  <Card.Body className="text-capitalize text-left">{element.body}</Card.Body>
								</Accordion.Collapse>
							</Card>;
				})
			}
